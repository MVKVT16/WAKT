...
function post() {
  ...
  $result = json_decode(
          file_get_contents('https://www.wikidata.org/w/api.php?action=wbgetentities&props=sitelinks&format=json&ids=' . $list), true
  );
  ...
  $response = json_decode($client->makeOAuthCall(
                  $accessToken, $wiki . 'w/api.php', true, $apiParams
          ), true);
  ...
  $summary = 'Från WAKT: koordinater för konstverket "' . $title
          . '" har ändrats till (' . $lat . ', ' . $long . ').';
  $apiParams = array(
      'action' => 'edit',
      'title' => $pageTitle,
      'text' => $newWikiText,
